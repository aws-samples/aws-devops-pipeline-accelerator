...
module "codebuild_iam_role" {
  ...
  project_name               = var.project_name
  codepipeline_iam_role_name = var.create_new_role == true ? "${var.project_name}-codebuild-role" : var.codepipeline_iam_role_name
  iam_policy_name            = var.create_new_role == true ? "${var.project_name}-codebuild-policy" : var.codepipeline_iam_role_name
  source_repository_name     = var.source_repo_name
  common_repository_name     = var.common_repo_name
  kms_key_arn                = module.codepipeline_kms.arn
...
module "codebuild_terraform" {
  ...
  project_name                        = var.project_name
  role_arn                            = module.codebuild_iam_role.role_arn
...
module "codepipeline_iam_role" {
  ...
  iam_policy_name            = var.create_new_role == true ? "${var.project_name}-codepipeline-policy" : var.codepipeline_iam_role_name
  source_repository_name     = var.source_repo_name
  common_repository_name     = var.common_repo_name
  kms_key_arn                = module.codepipeline_kms.arn
  s3_bucket_arn              = module.s3_artifacts_bucket.arn
...
module "codepipeline_terraform" {
  ...
  common_repo_name      = var.common_repo_name
  common_repo_branch    = var.common_repo_branch
  s3_bucket_name        = module.s3_artifacts_bucket.bucket
  codepipeline_role_arn = module.codepipeline_iam_role.role_arn
